...
export const VALIDATION_ERRORS = {
    ...
    MIN_GREATER_THAN_MAX: (field) => `Minimum ${field} cannot be greater than maximum ${field}`,
...
export const validateString = (value, fieldName, config = {}) => {
    ...
    if (config.minLength && trimmedValue.length < config.minLength) {
        errors.push(VALIDATION_ERRORS.INVALID_LENGTH(fieldName, config.minLength, config.maxLength || 'unlimited'));
    }

    if (config.maxLength && trimmedValue.length > config.maxLength) {
        errors.push(VALIDATION_ERRORS.INVALID_LENGTH(fieldName, config.minLength || 0, config.maxLength));
    }
    ...
    const sanitizedValue = config.sanitize ? sanitizeInput(trimmedValue) : trimmedValue;
...
// Phone validation
export const validatePhone = (phone, required = true) => { // Đặt required = true theo yêu cầu
...
export const validateNumber = (value, fieldName, config = {}) => {
    ...
    if (config.min !== undefined && numValue < config.min) {
        errors.push(VALIDATION_ERRORS.INVALID_RANGE(fieldName, config.min, config.max || 'unlimited'));
    }

    if (config.max !== undefined && numValue > config.max) {
        errors.push(VALIDATION_ERRORS.INVALID_RANGE(fieldName, config.min || 'unlimited', config.max));
    }
...
// Enum validation
export const validateEnum = (value, fieldName, enumValues, required = false) => {
    if (required && !value) {
        return {
            isValid: false,
            errors: [VALIDATION_ERRORS.REQUIRED_FIELD(fieldName)],
...
export const validatePagination = (page, limit) => {
    ...
    return {
        page: pageValidation.sanitizedValue || VALIDATION_CONFIG.PAGINATION.DEFAULT_PAGE,
        limit: limitValidation.sanitizedValue || VALIDATION_CONFIG.PAGINATION.DEFAULT_LIMIT,
...
export const validateRange = (minValue, maxValue, fieldName) => {
    ...
    if (minValidation.isValid && maxValidation.isValid &&
        minValidation.sanitizedValue !== null && maxValidation.sanitizedValue !== null &&
...
//   },
  ...
  is_active: {
    ...
    type: {
      type: 'enum',
      required: true,
      values: ['REMINDER', 'PAYMENT_SUCCESS', 'CONFIRMATION', 'WARNING', 'ERROR'],
    },
    account_id: {
      type: 'number',
      min: 1,

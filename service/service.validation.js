...
export const validateServiceData = (data, imageFile, isPartial = false) => {
  ...
  if (!isPartial || data.setup_time !== undefined) {
    ...
    sanitized.setup_time = data.setup_time !== undefined ? Number(data.setup_time) : undefined;
  }

  if (!isPartial || data.is_available !== undefined) {
    const availableValidation = validateBoolean(parseBoolean(data.is_available), 'Is available');
    errors.push(...availableValidation.errors);
    sanitized.is_available = data.is_available !== undefined ? parseBoolean(data.is_available) : undefined;
  }

  if (!isPartial || data.is_active !== undefined) {
    const activeValidation = validateBoolean(parseBoolean(data.is_active), 'Is active');
    errors.push(...activeValidation.errors);
    sanitized.is_active = data.is_active !== undefined ? parseBoolean(data.is_active) : undefined;
  }

  if (!isPartial || data.service_type_id !== undefined) {
    const typeIdValidation = parseAndValidateId(data.service_type_id, 'Service Type ID');
    if (!typeIdValidation) errors.push('Invalid Service Type ID.');
    sanitized.service_type_id = data.service_type_id !== undefined ? Number(data.service_type_id) || null : undefined;
  }
...
export const validateServiceFilters = (filters) => {
  ...
  if (paginationValidation.errors.length > 0) return createValidationResult(false, paginationValidation.errors);

  const errors = [];

  if (minSetupTime !== undefined) {
    const setupMinValidation = validateNumber(minSetupTime, 'Minimum setup time', {
      min: VALIDATION_CONFIG.SETUP_TIME.MIN || 0,
    });
  ...
  if (maxSetupTime !== undefined) {
    const setupMaxValidation = validateNumber(maxSetupTime, 'Maximum setup time', {
      min: VALIDATION_CONFIG.SETUP_TIME.MIN || 0,
    });
  ...
  if (minPrice !== undefined) {
    const priceMinValidation = validateNumber(minPrice, 'Minimum price', { min: 0 });
    errors.push(...priceMinValidation.errors);
  }

  if (maxPrice !== undefined) {
    const priceMaxValidation = validateNumber(maxPrice, 'Maximum price', { min: 0 });
    errors.push(...priceMaxValidation.errors);
  }

  if (minRating !== undefined) {
    const ratingMinValidation = validateNumber(minRating, 'Minimum rating', { min: 0, max: 5 });
    errors.push(...ratingMinValidation.errors);
  }

  if (maxRating !== undefined) {
    const ratingMaxValidation = validateNumber(maxRating, 'Maximum rating', { min: 0, max: 5 });
    errors.push(...ratingMaxValidation.errors);
  }

  if (serviceTypeId !== undefined) {
    const typeIdValidation = parseAndValidateId(serviceTypeId, 'Service Type ID');
    if (!typeIdValidation) errors.push('Invalid Service Type ID.');
  }

  if (createdFrom || createdTo) {
    const dateValidation = validateDateRange(createdFrom, createdTo, 'Created Date');
    errors.push(...dateValidation.errors);
  }

  if (updatedFrom || updatedTo) {
    const dateValidation = validateDateRange(updatedFrom, updatedTo, 'Updated Date');
    errors.push(...dateValidation.errors);
  }
  ...
  return createValidationResult(true, [], {
    ...
    serviceTypeId: serviceTypeId !== undefined ? Number(serviceTypeId) : undefined,
    createdFrom,
    createdTo,
    updatedFrom,
    updatedTo,

...
export const validateEventServiceData = (serviceData) => {
  ...
  if (serviceData.event_id) {
    const eventIdValidation = parseAndValidateId(serviceData.event_id, "Event ID");
...
export const checkVariationAvailability = async (variation_id, scheduled_time, duration_hours, tx = prisma) => {
  try {
    // Validate inputs
    ...
    const requestedEnd = new Date(requestedStart.getTime() + duration_hours * 3600 * 1000);
...
export const createEventService = async (serviceData, user = null) => {
  try {
    const { event_id, service_id, variation_id, quantity = 1, custom_price, notes, status = 'CONFIRMED', scheduled_time, duration_hours } = serviceData;
    ...
    if (variation_id) {
      ...
      if (!variation || !variation.is_active || variation.service_id !== Number(service_id)) {
        return createValidationResult(false, ["Variation not found, inactive, or does not belong to the specified service"]);
      }

      // Check availability if scheduled_time and duration_hours are provided
      if (scheduled_time && duration_hours) {
        const availabilityCheck = await checkVariationAvailability(variation_id, scheduled_time, duration_hours);
    ...
    const newEventService = await prisma.eventService.create({
      ...
      include: {
        event: { select: { event_id: true, event_name: true, account_id: true } },
...
export const updateEventService = async (eventServiceId, updateData) => {
  try {
    const validEventServiceId = parseAndValidateId(eventServiceId, "Event Service ID");
    ...
    const { event_id, service_id, variation_id, quantity, custom_price, notes, status, scheduled_time, duration_hours } = updateData;
    ...
    if (variation_id && variation_id !== existingEventService.variation_id) {
      ...
      if (!variation || !variation.is_active || variation.service_id !== Number(service_id || existingEventService.service_id)) {
        return createValidationResult(false, ["Variation not found, inactive, or does not belong to the specified service"]);
      }
      ...
      const effectiveScheduledTime = scheduled_time || existingEventService.scheduled_time;
      const effectiveDurationHours = duration_hours || existingEventService.duration_hours;
      if (effectiveScheduledTime && effectiveDurationHours) {
        const availabilityCheck = await checkVariationAvailability(variation_id, effectiveScheduledTime, effectiveDurationHours);
        if (!availabilityCheck.isValid) {
          return availabilityCheck;
        }
      }
    ...
    if (variation_id !== undefined) updateFields.variation_id = variation_id ? Number(variation_id) : null;
    if (quantity !== undefined) updateFields.quantity = Number(quantity);
    if (custom_price !== undefined) updateFields.custom_price = custom_price ? Number(custom_price) : null;
    if (notes !== undefined) updateFields.notes = notes?.trim() || null;
    if (status !== undefined) updateFields.status = status;
    if (scheduled_time !== undefined) updateFields.scheduled_time = scheduled_time ? new Date(scheduled_time) : null;
...
export const getAllEventServices = async (filters = {}) => {
  try {
    const { event_id, service_id, variation_id, status, page = 1, limit = 20, sortBy = 'created_at', sortOrder = 'asc' } = filters;
    ...
    if (variation_id) where.variation_id = parseAndValidateId(variation_id, "Variation ID");

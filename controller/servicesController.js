...
import { validateServiceData, validateServiceFilters } from '../service/service.validation.js';
...
export const getAllServices = async (req, res) => {
  try {
    ...
    if (!result.isValid)
      return sendResponse(res, 500, result.errors);
    return sendResponse(res, 200, "Services retrieved successfully", result.data);
  } catch (error) {
    return sendResponse(res, 500, error.message);
  }
};
...
export const getServiceById = async (req, res) => {
  try {
    const { id } = req.params;
    if (!isValidId(id))
      return sendResponse(res, 400, "Invalid service ID");

    const result = await getServiceByIdQuery(parseInt(id), req.query);
    if (!result.isValid)
      return sendResponse(res, 404, result.errors);
    return sendResponse(res, 200, "Service retrieved successfully", result.data);
  } catch (error) {
    return sendResponse(res, 500, error.message);
  }
};
...
export const updateService = async (req, res) => {
  try {
    ...
    const result = await updateServiceQuery(parseInt(id), validation.sanitizedData, req.file);
    if (!result.isValid)
      return sendResponse(res, 500, result.errors);
...
export const bulkUpdateServices = [
  ...
  async (req, res) => {
    try {
      ...
      const invalidInputs = serviceIds.filter(id => id === null || id === undefined || id === "");
      if (invalidInputs.length) {
        return sendResponse(res, 400, `Invalid service IDs: ${JSON.stringify(invalidInputs)}`);
      }

      const numericServiceIds = serviceIds.map(id => Number(id));
      const invalidIds = numericServiceIds.filter(id => !isValidId(id));
      if (invalidIds.length) {
        return sendResponse(res, 400, `Invalid service IDs: ${JSON.stringify(invalidIds)}`);
      }
      ...
      const result = await bulkUpdateServicesQuery(numericServiceIds, validation.sanitizedData);
      if (!result.isValid) {
...
export const exportServices = async (req, res) => {
  try {
    ...
    const result = await getAllServicesQuery({ ...req.query, limit: 1000, includeStats: true });
    if (!result.isValid)
      return sendResponse(res, 500, result.errors);
